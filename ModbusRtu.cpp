 ...
 * @ingroup setup
 */
Modbus::Modbus(uint8_t u8id, Stream& port, uint8_t u8txenpin) // @suppress("Class members should be properly initialized")
{
    this->port = &port;
    this->u8id = u8id;
 ...
 */
Modbus::Modbus(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin) // @suppress("Class members should be properly initialized")
{
    this->u8id = u8id;
    this->u8txenpin = u8txenpin;
    this->u16timeOut = 1000;
